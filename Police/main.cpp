...
void add_crime(std::map<std::string, std::list<Crime>>& base) {
	...
	base[license_plate].push_back(Crime(number, place, time(NULL)));
}
...
void print_range(std::map<std::string, std::list<Crime>>& base) {
	...
	for (std::map<std::string, std::list<Crime>>::iterator plate = base.lower_bound(first);
		plate != base.upper_bound(last); ++plate) {

		cout << plate->first << ":\n";
		for (std::list<Crime>::iterator crime = plate->second.begin();
			crime != plate->second.end(); ++crime) {
